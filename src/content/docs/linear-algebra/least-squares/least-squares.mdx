...
<TabItem label="Proof">
For any $\xv$, write $\Av\xv - \bv = \Av(\xv - \hat\xv) + (\Av\hat\xv - \bv)$ where $\hat\xv$ solves the normal equations. The cross term vanishes because
$$
(\Av(\xv-\hat\xv))^{\rm T}(\Av\hat\xv - \bv) = (\xv - \hat\xv)^{\rm T}\Av^{\rm T}(\Av\hat\xv - \bv) = 0,
$$
using $\Av^{\rm T}(\Av\hat\xv - \bv) = 0$. Hence $\lVert \Av\xv - \bv \rVert^2 = \lVert \Av(\xv-\hat\xv) \rVert^2 + \lVert \Av\hat\xv - \bv \rVert^2 \ge \lVert \Av\hat\xv - \bv \rVert^2$, with equality when $\Av\xv = \Av\hat\xv$. So every solution of the normal equations is a minimizer, and conversely a minimizer makes the gradient vanish. When the columns are independent, $\Av\xv = 0 \Rightarrow \xv = 0$, so $\Av^{\rm T}\Av\xv = 0 \Rightarrow \xv^{\rm T}\Av^{\rm T}\Av\xv = \lVert \Av\xv \rVert^2 = 0 \Rightarrow \xv = 0$; thus $\Av^{\rm T}\Av$ is invertible.
</TabItem>
</Tabs>
</Aside>
