---
title: The pseudoinverse and minimum-norm solution
sidebar:
  order: 2
---
...
![A maps the row space onto the column space and the nullspace to zero; the pseudoinverse maps the column space back onto the row space and the left nullspace to zero](/assets/images/linear-algebra/least-squares/pseudoinverse-subspaces.svg)
...
<TabItem label="Proof">
From the SVD, $\xv^{+} = \Av^{+}\bv = \sum_{i \le r} \tfrac{1}{\sigma_i}(\uv_i^{\rm T}\bv)\,\vv_i$ is a combination of $\vv_1, \dots, \vv_r$, hence lies in the row space. Any other least-squares solution differs from it by a vector in the nullspace: $\xv = \xv^{+} + \nv$ with $\nv \in N(\Av)$ (changing $\xv$ within $N(\Av)$ leaves $\Av\xv$, and therefore the residual, unchanged). Since the row space and nullspace are orthogonal complements, $\xv^{+} \perp \nv$, so by the Pythagorean identity
$$
\lVert \xv \rVert^2 = \lVert \xv^{+} \rVert^2 + \lVert \nv \rVert^2 \ge \lVert \xv^{+} \rVert^2,
$$
with equality only when $\nv = 0$. Thus $\xv^{+}$ is the unique minimum-norm solution.
</TabItem>
</Tabs>
</Aside>
