...
<Aside icon="seti:smarty" title="Claim: Orthogonal transformations">
<Tabs>
  <TabItem label="Statement">
    For any vector $\xv$, $\lVert \Qv\xv\rVert = \lVert \xv\rVert$, i.e., $\Qv$ does not change the length of $\xv$.
  </TabItem>
  <TabItem label="Proof">
    See that $\lVert \Qv\xv\rVert^2 = (\Qv\xv)^{\rm T}(\Qv\xv) = \xv^{\rm T}\Qv^{\rm T}\Qv\xv = \xv^{\rm T}\xv = \lVert \xv\rVert^2$, since $\Qv^{\rm T}\Qv = \Iv$. Taking square roots gives $\lVert \Qv\xv\rVert = \lVert \xv\rVert$.
  </TabItem>
</Tabs>
</Aside>
