...
sidebar:
  order: 4
---
import EigenvectorRotation from '../../../../components/widgets/EigenvectorRotation.tsx';

<Aside type="tip" icon="open-book" title="Definition: Eigenvalues and Eigenvectors">
...
Naturally, eigenvalues are defined <span class="text-accent">only for square matrices</span> because for non-square matrices, the transformation changes the dimensions of the resulting vector, hence $\Av \vv \neq \lambda \vv$ for any pair $(\lambda, \vv)$. An $n\times n$ matrix has at most $n$ linearly independent eigenvectors, but it need not have that many. A matrix with $n$ linearly independent eigenvectors is called <span class="text-accent">diagonalizable</span>; matrices that fall short (e.g., a Jordan block) are called <span class="text-accent">defective</span>.
...
<EigenvectorRotation client:visible />
...
<Aside icon="seti:smarty" title="Claim: Invertibility">
<Tabs>
...
<Aside icon="seti:smarty" title="Claim: Similarity">
<Tabs>
  <TabItem label="Statement">
