...
3. Now let $\cM$ be the collection of all $\Pr^*$-measurable sets. It is a standard result in measure theory that $\cM$ is a $\sigma$-field and that $\Pr^*$ restricted to $\cM$ is a countably additive measure. Thus, $\Pr^*$ is a probability measure on the $\sigma$-field $\cM$. What we need now is a relationship between $\sigma(\cF_0)$ and $\cM$. More precisely, we need to show that $\cF_0 \subseteq \cM$. Since $\cM$ is a $\sigma$-field and contains $\cF_0$, it must also contain the $\sigma$-field generated by $\cF_0$, i.e., $\sigma(\cF_0) \subseteq \cM$. Thus, $\Qv = \Pr^*|_{\sigma(\cF_0)}$ will be our required probability measure.
